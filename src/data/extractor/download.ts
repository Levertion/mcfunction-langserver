...
export async function getPathToJar(tempdir: string, currentversion: string): Promise<JarInfo> {
...
export async function downloadJar(currentversion: string, tmpDirName: string): Promise<JarInfo> {
    const versionInfo = await getLatestVersionInfo();
    if (versionInfo.id !== currentversion) {
        const singleVersion: SingleVersionInformation = await requestPromise(versionInfo.url, {
            json: true,
        }).promise();
...
export interface JarInfo { jarPath: string; version: string; }
...
async function getLatestVersionInfo(): Promise<VersionInfo> {
    const manifest: VersionsManifest =
        await requestPromise("https://launchermeta.mojang.com/mc/game/version_manifest.json", { json: true }).promise();
...
function findVersion(version: string, manifest: VersionsManifest): VersionInfo {
    return manifest.versions.find((verInfo) => verInfo.id === version) as VersionInfo;
}
