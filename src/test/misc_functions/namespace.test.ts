...
import { exactifyNamespace, namespacesEqual } from "../../misc_functions/namespace";
...
describe("Namespace Functions (Misc)", () => {
    ...
    describe("namespacesEqual()", () => {
        it("should return true if the names are equal", () => {
            ...
            assert(namespacesEqual(name, { namespace: "namespace", path: "testpath" }));
        });
        it("should return false if the namespaces aren't equal", () => {
            const name: NamespacedName = { namespace: "namespace1", path: "testpath" };
            assert(!namespacesEqual(name, { namespace: "namespace2", path: "testpath" }));
        });
        it("should return false if the paths aren't equal", () => {
            const name: NamespacedName = { namespace: "namespace", path: "testpath1" };
            assert(!namespacesEqual(name, { namespace: "namespace", path: "testpath2" }));
        });
    });
});
