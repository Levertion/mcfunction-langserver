...
import { SwizzleParser } from "../../../../parse/parsers/minecraft/swizzle";
...
describe("Swizzle", () => {
    ...
    describe("getSuggestions()", () => {
        [
            ...
            const out = parser.getSuggestions(v[0] as string);
            assert.ok(
                out.map((v2) => v2.value).every(
                    (v2) => (v[1] as string[]).indexOf(v2) !== -1,
                ),
                `[${out.map((v2) => v2.value).join(", ")}] does not match [${(v[1] as string[]).join(", ")}]`,
            );
        }));
    });
});
